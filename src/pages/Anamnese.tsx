...
const anamneseSchema = z.object({
  ...
  documento_frente_url: z.string().optional(),
  documento_verso_url: z.string().optional(),
...
const Anamnese: React.FC = () => {
  ...
  const [fotoUrlRecord, setFotoUrlRecord] = useState<string | null>(null);
  ...
  const [isUploadingFotoRecord, setIsUploadingFotoRecord] = useState(false);
  const docFrenteInputRef = useRef<HTMLInputElement>(null);
  const docVersoInputRef = useRef<HTMLInputElement>(null);
  const fotoRecordInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    const fetchAnamnese = async () => {
      ...
      if (!error && data) {
        ...
        if (data.foto_url) setFotoUrlRecord(data.foto_url);
        ...
        const mapped: AnamneseFormData = {
          ...
          documento_frente_url: data.documento_frente_url || '',
          documento_verso_url: data.documento_verso_url || '',
          foto_url: data.foto_url || '',
        };
        setFormData(mapped);
        setExistingAnamnese(mapped);

        // Verificar se faltam novos campos obrigatórios (documento e assinatura)
        const incomplete = !mapped.documento_frente_url || !mapped.assinatura;
  ...
  const calculateProgress = (): number => {
    ...
    // 10% para foto da ficha
    const fotoProgress = formData.foto_url ? 10 : 0;

    // 20% para assinatura
    const signatureProgress = formData.assinatura ? 20 : 0;

    return Math.min(100, Math.round(baseProgress + healthProgress + consentProgress + fotoProgress + signatureProgress));
  };
  ...
  const handleDocumentUpload = async (
    ...
    try {
      ...
      // Salvar URL na tabela anamneses sem depender de upsert com onConflict que está falhando
      ...
      if (dbError) {
        ...
        if (side === 'frente') {
          setDocumentoFrenteUrl(publicUrl);
          updateField('documento_frente_url', publicUrl);
        } else {
          setDocumentoVersoUrl(publicUrl);
          updateField('documento_verso_url', publicUrl);
        }
  ...
  const handleAnamneseFotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !user) return;
    ...
    setIsUploadingFotoRecord(true);
    try {
      const fileExt = file.name.split('.').pop();
      const fileName = `${user.id}/personal_photo_${Date.now()}.${fileExt}`;
      ...
      // Salvar URL no banco na tabela anamneses
      const { data: existing } = await supabase
        .from('anamneses')
        .select('user_id')
        .eq('user_id', user.id)
      ...
      if (dbError) {
        toast.error('Foto enviada, mas erro ao salvar no registro: ' + dbError.message);
      } else {
        setFotoUrlRecord(publicUrl);
        updateField('foto_url', publicUrl);
        toast.success('Foto da ficha atualizada!');
      }
    } catch (error) {
      console.error('Erro no upload da foto:', error);
      toast.error('Erro ao enviar foto');
    } finally {
      setIsUploadingFotoRecord(false);
      if (fotoRecordInputRef.current) fotoRecordInputRef.current.value = '';
    }
  };
        ...
        <div className="min-h-screen py-4 md:py-6">
          <div className="container max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              {/* Status de Saúde */}
              <div className={`p-3 rounded-xl border ${hasContraindicacoes
              ...
              {/* Substâncias */}
              <div className={`p-3 rounded-xl border ${formData.sem_vicios
                ? 'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800'
              ...
              {/* Experiência */}
              <div className={`p-3 rounded-xl border ${formData.ja_consagrou
              ...
              {/* Permanência */}
              <div className={`p-3 rounded-xl border ${formData.aceite_permanencia
                ? 'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800'
              ...
              {/* Uso de Imagem */}
              <div className={`p-3 rounded-xl border ${formData.aceite_uso_imagem
                ? 'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800'
                : 'bg-slate-50 dark:bg-slate-950/30 border-slate-200 dark:border-slate-800'
                }`}>
                ...
                <CardContent>
                  ...
                  {formData.foto_url && (
                    <div className="py-3 border-b border-border last:border-0">
                      <p className="text-xs text-muted-foreground mb-2">Foto da Ficha</p>
                      <img
                  ...
                  <div className="grid grid-cols-2 gap-4 py-3 border-b border-border">
                    <div>
                      ...
                      {formData.documento_frente_url ? (
                        <img src={formData.documento_frente_url} alt="Frente" className="w-full h-24 rounded border object-cover" />
                      ) : (
                        <div className="w-full h-24 rounded border bg-muted flex items-center justify-center">
                          <IdCard className="w-6 h-6 text-muted-foreground opacity-20" />
                        </div>
                      )}
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Doc. Verso</p>
                      {formData.documento_verso_url ? (
                        <img src={formData.documento_verso_url} alt="Verso" className="w-full h-24 rounded border object-cover" />
                      ) : (
                        <div className="w-full h-24 rounded border bg-muted flex items-center justify-center">
                          <IdCard className="w-6 h-6 text-muted-foreground opacity-20" />
                        </div>
          ...
          <div className="flex justify-center gap-2 mb-8">
            {steps.map((s) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-full text-sm font-body transition-all cursor-pointer hover:scale-105 ${step === s.number
                ...
                <CardContent className="space-y-5 md:space-y-4">
                  ...
                  <div className="flex flex-col items-center gap-3 pb-4 border-b">
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      Clique no ícone para adicionar sua foto de perfil<br />
                      (aparecerá nas suas partilhas)
                    </p>
                  </div>

                  {/* Foto para a Ficha - Requisito do Usuário */}
                  <div className="py-4 border-y border-border">
                    <h4 className="font-medium mb-3 flex items-center gap-2 text-sm">
                      <Camera className="w-4 h-4 text-primary" />
                      Foto para a Ficha de Anamnese *
                    </h4>
                    <div className="flex flex-col items-center gap-3">
                      <div
                        className="relative border-2 border-dashed border-border rounded-2xl p-2 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-colors w-full max-w-[200px] aspect-square"
                        onClick={() => !isUploadingFotoRecord && fotoRecordInputRef.current?.click()}
                      >
                        {fotoUrlRecord ? (
                          <>
                            <img
                          ...
                          <>
                            ...
                            <span className="text-xs text-muted-foreground text-center">Clique para tirar ou<br />selecionar uma foto</span>
                          </>
                        )}
                      </div>
                      <input
                        ref={fotoRecordInputRef}
                        type="file"
                        accept="image/*"
                        capture="user"
                        className="hidden"
                        onChange={handleAnamneseFotoUpload}
                      />
                      {errors.foto_url && <p className="text-xs font-medium text-destructive">{errors.foto_url}</p>}
                    </div>
                  </div>
